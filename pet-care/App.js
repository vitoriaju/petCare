...
import {
  View, Text, TextInput, Button, StyleSheet, TouchableOpacity, ImageBackground
} from 'react-native';
...
function AuthScreen({ navigation }) {
  ...
  const handlePasswordReset = async () => {
    ...
    updatedUsers[userIndex].password = hashPassword(password);
    setUsers(updatedUsers);
    await saveUsers(updatedUsers);
  ...
  const formatPhone = (value) => {
    return value
      ...
      .replace(/(\d{5})(\d)/, '$1-$2') // Traço no número
      .slice(0, 15); // Limite de tamanho
  };
